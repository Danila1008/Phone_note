...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree = ttk.Treeview(self, columns=('ID', 'name', 'tel', 'email', 'wage'),
        ...
        self.tree.column("wage", width=150, anchor=tk.CENTER)
        # подписи колонок
        self.tree.heading("ID", text='ID')
        self.tree.heading("name", text='ФИО')
        self.tree.heading("tel", text='Телефон')
        self.tree.heading("email", text='E-mail')
        self.tree.heading("wage", text='заработная плата')
        # упаковка
    ...
    # добавление данных
    def records(self, name, tel, email, wage):
        self.db.insert_data(name, tel, email, wage)
        self.view_records()

    # обновление (изменение) данных
    def update_record(self, name, tel, email, wage):
        self.db.c.execute('''UPDATE db SET name=?, tel=?, email=?, wage=? WHERE ID=?''', (name, tel, email, wage, self.tree.set(self.tree.selection()[0], '#1')))
        self.db.conn.commit()
        self.view_records()
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        label_suma = tk.Label(self, text='wage')
        label_suma.place(x=50, y=140)
...
class DB:
    def __init__(self):
        ...
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS db (id integer primary key, name text, tel text, email text, wage text)''')
        # сохранение изменений БД
        self.conn.commit()

    # метод добавления в БД
    def insert_data(self, name, tel, email, wage):
        self.c.execute('''INSERT INTO db (name, tel, email, wage) VALUES (?, ?, ?, ?)''',
                       (name, tel, email, wage))
        self.conn.commit()
